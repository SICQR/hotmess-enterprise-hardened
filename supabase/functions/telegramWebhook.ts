...
serve(async (req: Request) => {
  ...
  await supabase.from('bot_logs').insert({ chat_id: chatId, message, created_at: new Date().toISOString() })
  // Determine response
  let response = ''
  if (/\/start/.test(message)) {
    ...
    const trackRes = await fetch(`${supabaseUrl}/rest/v1/rpc/get_current_track`)
