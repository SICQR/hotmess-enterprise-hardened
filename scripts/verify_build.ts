...
check("Environment file", () => {
  const envExists =
    fs.existsSync(path.join(ROOT_DIR, ".env.local")) ||
...
check("Source directory", () => {
  ...
  const missing = requiredFiles.filter(
    (f) => !fs.existsSync(path.join(srcPath, f)),
  );
...
check(
  ...
  () => {
    ...
    const missing = requiredPages.filter(
      (p) => !fs.existsSync(path.join(pagesPath, p)),
    );
...
check(
  ...
  () => {
    ...
    if (!fs.existsSync(componentsPath))
      return "src/components/ directory not found";
    ...
    const missing = requiredComponents.filter(
      (c) => !fs.existsSync(path.join(componentsPath, c)),
    );
...
check(
  ...
  () => {
    ...
    const missing = essentialComponents.filter(
      (c) => !fs.existsSync(path.join(uiPath, c)),
    );

    return (
      missing.length === 0 || `Missing UI components: ${missing.join(", ")}`
    );
  },
  false,
);
...
check(
  ...
  () => {
    ...
    return (
      fs.existsSync(prdPath) ||
      "PRD.md not found (recommended for documentation)"
    );
  },
  false,
);
...
if (failCount > 0) {
  console.log(
    "❌ Critical checks failed. Please fix the issues above before launching.\n",
  );
