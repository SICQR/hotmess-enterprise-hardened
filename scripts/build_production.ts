...
const steps: BuildStep[] = [
  ...
  },
  {
    ...
    description: "Pre-build integrity checks",
    status: "pending",
  },
  {
  ...
  {
    name: "Vite Build",
    description: "Bundle production assets",
    status: "pending",
  },
  {
    name: "Docker Generation",
    description: "Regenerate Dockerfile",
    status: "pending",
  },
  {
    name: "Kubernetes Generation",
    description: "Regenerate k8s manifests",
    status: "pending",
  },
  {
    name: "Health Check",
...
function displayProgress() {
  ...
  steps.forEach((step, index) => {
    ...
    console.log(
      `${color}${icon} ${index + 1}. ${step.name}${reset}${durationStr}`,
    );
...
function writeSummary(totalDuration: number) {
  try {
    ...
    console.warn(
      "⚠️  Failed to write build summary:",
...
async function runMigrations(): Promise<boolean> {
  ...
  if (!supabaseUrl || !supabaseKey) {
    console.log(
      "   ⚠ Supabase credentials not found (validating SQL syntax only)",
    );
