...
type Intent = 'onboarding' | 'rides' | 'eats' | 'radio' | 'shop' | 'earn' | 'safety' | 'unknown'
...
export function ConciergeWidget() {
  ...
  const classifyIntent = (text: string): Intent => {
    ...
    if (lower.includes('ride') || lower.includes('uber') || lower.includes('lyft')) return 'rides'
    if (lower.includes('food') || lower.includes('eat') || lower.includes('doordash')) return 'eats'
    if (lower.includes('radio') || lower.includes('listen') || lower.includes('music')) return 'radio'
    if (lower.includes('shop') || lower.includes('buy') || lower.includes('store')) return 'shop'
    if (lower.includes('earn') || lower.includes('affiliate') || lower.includes('money')) return 'earn'
    if (lower.includes('help') || lower.includes('crisis') || lower.includes('suicide') || lower.includes('harm')) return 'safety'
    return 'unknown'
  }
  ...
  const handleSend = async () => {
    ...
    const assistantMessage: Message = { role: 'assistant', content: response, intent }
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-border bg-accent text-accent-foreground">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="border-accent-foreground/20">CONCIERGE</Badge>
        </div>
        <Button
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4" role="log" aria-live="polite" aria-atomic="false">
        {messages.length === 1 && (
          <div className="grid grid-cols-2 gap-2 mb-4" role="group" aria-label="Quick action buttons">
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="bg-muted rounded-lg p-3 text-muted-foreground" aria-label="AI is typing">
              Thinking...
            </div>
          </div>
        )}
