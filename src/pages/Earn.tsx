...
export default function Earn() {
  ...
  async function submitApplication() {
    try {
      await supabase
        .from("affiliates")
        .insert({ handle, community, created_at: new Date().toISOString() });
      const modChat = import.meta.env.VITE_TELEGRAM_MOD_CHAT_ID;
      if (modChat) {
        await sendTelegramMessage(
          modChat,
          `New affiliate application from ${handle}: ${community}`,
        );
      }
      setSubmitted(true);
    } catch (err) {
  ...
  return (
    <div className="px-4 py-12 max-w-4xl mx-auto">
      ...
      <p className="mb-6 opacity-80">
        Earn while keeping it messy. Our men‑only affiliate program pays you to
        spread the HOTMESS message.
      </p>
      <h2 className="text-2xl font-bold mb-2">How It Works</h2>
      <ul className="list-disc ml-8 space-y-2 mb-4">
        <li>
          Apply using your chosen handle and share a bit about your community.
        </li>
        <li>
          Receive your unique affiliate link powered by the HMAC-secure browser
          crypto API.
        </li>
      ...
      <div className="mt-12 space-y-4">
        ...
        <p className="opacity-80">
          As an affiliate you can drive sales by promoting exclusive drops,
          events and aftercare. Listen to our shows for promo codes and shop the
          latest to stay ahead.
        </p>
        <div className="flex flex-wrap gap-4">
          <button
            onClick={() =>
              alert(
                "Tune into HOTMESS Radio for codes you can share with your followers!",
              )
            }
          ...
          <button
            onClick={() =>
              alert(
                "Browse the shop and familiarise yourself with our products before promoting them!",
              )
            }
            className="bg-red-700 hover:bg-red-800 text-white px-6 py-3 rounded font-bold"
          >
          ...
          <button
            onClick={() =>
              alert(
                "Need aftercare insights? Visit the Care page for resources you can share.",
              )
            }
            className="bg-red-700 hover:bg-red-800 text-white px-6 py-3 rounded font-bold"
          >
