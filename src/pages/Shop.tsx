...
export default function Shop() {
  const [items, setItems] = useState<UnifiedProduct[]>(products as unknown as UnifiedProduct[])
  ...
  return (
    <div className="px-4 py-12 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {items.map(product => (
          <div key={product.id} className="bg-[#1a1a1a] rounded shadow overflow-hidden">
            <img
              src={product.image}
              alt={product.alt}
              ...
              <button
                ...
                onClick={() => {
                  if (typeof window !== 'undefined') {
                    const consented = sessionStorage.getItem('age-verified') === 'true'
                    if (consented) {
                      alert('Added to bag!')
                    } else {
      ...
      <div className="mt-12 text-center space-y-4">
        ...
        <p className="opacity-80 max-w-xl mx-auto">
          Earn more and stay engaged – scan QR beacons to unlock exclusive drops, tune into HOTMESS Radio for show‑only discount codes, and join our affiliate programme to earn commissions on every sale.
        </p>
        <div className="flex flex-wrap justify-center gap-4">
          <button
            onClick={() => alert('Tune into HOTMESS Radio for exclusive discount codes!')}
            className="bg-red-700 hover:bg-red-800 text-white px-6 py-3 rounded font-bold"
          >
            Listen to Radio
          </button>
          <button
            onClick={() => alert('Apply to our Affiliate Gang and earn commissions!')}
            className="bg-red-700 hover:bg-red-800 text-white px-6 py-3 rounded font-bold"
          >
            Join Affiliate
          </button>
          <button
            onClick={() => alert('Need aftercare? Visit our Care page for resources.')}
            className="bg-red-700 hover:bg-red-800 text-white px-6 py-3 rounded font-bold"
          >
            Visit Care
          </button>
