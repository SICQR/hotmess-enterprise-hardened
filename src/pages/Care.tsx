...
export default function Care() {
    ...
    ;(async () => {
      try {
        if (feeling.trim().length > 0) {
          await supabase.from('checkins').insert({ message: feeling, created_at: new Date().toISOString() })
        }
        // Notify moderator via Telegram if chat ID and token are provided
        const modChat = import.meta.env.VITE_TELEGRAM_MOD_CHAT_ID
        if (modChat) {
          await sendTelegramMessage(modChat, `New aftercare check-in: ${feeling}`)
        }
      } catch (err) {
  ...
  return (
    <div className="px-4 py-12 max-w-4xl mx-auto">
      <h1 className="text-4xl font-extrabold mb-4">Hand N Hand: Care & Aftercare</h1>
      <p className="mb-8 opacity-80">
        Aftercare isn’t an afterthought. Here are some resources to decompress, hydrate, and reconnect.
        Not medical advice—just a friendly guide.
      </p>
      <ul className="list-disc ml-8 space-y-2 mb-8">
        <li>Drink a full glass of water and breathe deeply for two minutes.</li>
        <li>Check in with your partner(s): ask how they’re feeling and listen without judgment.</li>
      ...
      {!submitted ? (
        ...
        <p className="text-green-500">Thank you for checking in. We appreciate you taking care of yourself.</p>
      )}
      {/* Cross‑sell section linking to shop and radio */}
      <div className="mt-12 space-y-4">
        <h2 className="text-2xl font-bold">Next Steps</h2>
        <p className="opacity-80">
          Care doesn’t end here. Explore our HNH MESS line for products designed for aftercare, tune into Hand N Hand on Sundays for guided discussions, or join our affiliate programme to support others.
        </p>
        <div className="flex flex-wrap gap-4">
          ...
          <button
            onClick={() => alert('Listen to Hand N Hand on HOTMESS Radio for more aftercare discussions!')}
            className="bg-red-700 hover:bg-red-800 text-white px-6 py-3 rounded font-bold"
          >
            Listen to Radio
          </button>
